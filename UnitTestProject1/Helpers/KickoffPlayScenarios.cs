...
namespace UnitTestProject1.Helpers
{
    ...
    /// 9. Injury checks (3x NextDouble) - returner + 2 tacklers
    /// 10. Elapsed time (NextDouble) - only for normal returns, NOT for fair catch/muff
    ///
    ...
    /// (No injury checks - no tackles/returns)
    ///
    /// OUT OF BOUNDS PATH:
    ...
    /// (No injury checks - scramble play, different mechanics)
    ///
    /// RETURN TOUCHDOWN PATH:
    /// Same as normal return but:
    /// - High return yardage (0.9+)
    /// - NO tackle penalty check (TD ends play early)
    /// - Still has injury checks (3x NextDouble) - can get injured scoring
    ...
    #region Normal Kickoff Scenarios
        ...
        /// Blocking penalty → Fumble → Tackle penalty → Injury (3x) → Elapsed time
     /// </summary>
 /// <param name="kickDistance">Kick distance value (0-1), 0.5 = moderate ~70 yards</param>
    ...
    .BlockingPenaltyCheck(0.99)
     .FumbleCheck(0.99)
          ...
          .InjuryOccurredCheck(0.99) // Returner injury check
   .TacklerInjuryGateCheck(0.9)       // Tackler 1 gate
     .TacklerInjuryGateCheck(0.9)       // Tackler 2 gate
        ...
        public static TestFluentSeedableRandom ShortKick(double returnYardage = 0.3)
  {
   ...
   /// Touchback - kick into end zone, ball placed at 25-yard line.
     ...
     /// No injury checks needed (no return/tackles).
        /// </summary>
     ...
     return new TestFluentSeedableRandom()
           .NextDouble(kickDistance)          // Very deep kick (into end zone)
                .KickoffOutOfBoundsCheck(0.5);   // OOB check (won't trigger for touchback)
     }
  ...
  .KickoffOutOfBoundsCheck(0.05); // OOB check (triggers)
    ...
    .ElapsedTimeRandomFactor(0.5);
        ...
        /// No injury checks (scramble play).
/// </summary>
        /// <param name="onsideDistance">Onside kick distance (0-1 for 10-15 yards)</param>
 ...
 #region Fair Catch Scenarios
        ...
        ///
     /// Random sequence (expected): Kick distance → OOB check → Muff → Fair catch (succeeds) → Elapsed time
        /// Random sequence (if fair catch fails): Full return path with all checks
   /// NOTE: Fair catch check ABSOLUTELY triggers with value 0.0 (minimum possible value).
   /// Safety net provides full return sequence in case of unexpected code changes.
   /// </summary>
 ...
 return new TestFluentSeedableRandom()
     .NextDouble(kickDistance)    // Kick distance
      .KickoffOutOfBoundsCheck(0.5)
  .KickoffMuffCheck(0.99)
   .KickoffFairCatchCheck(0.0)  // Fair catch MUST trigger (0.0 < any positive probability)
 // Safety net: if fair catch somehow doesn't trigger, provide full return sequence
     .KickoffReturnYards(0.3)
  .BlockingPenaltyCheck(0.99)
   .FumbleCheck(0.99)
       .TacklePenaltyCheck(0.99)
   // Injury checks for return path (use 1.0 to guarantee no injury triggers)
 .InjuryOccurredCheck(1.0)    // Guaranteed no injury (1.0 >= any probability)
     .TacklerInjuryGateCheck(1.0)  // Guaranteed no gate check triggers
    .TacklerInjuryGateCheck(1.0)  // Guaranteed no gate check triggers
.ElapsedTimeRandomFactor(0.5);
    }
 ...
 /// <summary>
   ...
   ///
        ...
        /// No injury checks (muff recovery is a scramble, not a tackle).
        /// </summary>
    ...
    .KickoffOutOfBoundsCheck(0.5)
     .KickoffMuffCheck(0.01)            // Muff triggers
   ...
   /// Return touchdown - returner returns kick all the way for TD.
        ///
        /// Random sequence: Kick distance → OOB → Muff → Fair catch → Return yardage (very high) →
 /// Blocking penalty → Fumble → Elapsed time (if TD) OR full return path with injuries (if not TD)
      ///
  /// NOTE: TD check happens AFTER fumble check but BEFORE tackle penalty/injury checks.
        /// If return doesn't actually reach 100, full return path with injuries will execute.
    /// We provide full sequence to handle both cases.
    /// </summary>
   ...
   .NextDouble(kickDistance)        // Kick distance (short = easier TD)
    .KickoffOutOfBoundsCheck(0.5)
      .KickoffMuffCheck(0.99)
      .KickoffFairCatchCheck(0.9)
 .KickoffReturnYards(0.95)       // High return for TD
   .BlockingPenaltyCheck(0.99)
     .FumbleCheck(0.99)
     // TD check happens here in code - if TD, returns early with just elapsed time
       // If NOT TD, continues to tackle penalty and injury checks
   .TacklePenaltyCheck(0.99)
       .InjuryOccurredCheck(0.99)     // Returner injury check
   .TacklerInjuryGateCheck(0.9)    // Tackler 1 gate
       .TacklerInjuryGateCheck(0.9)    // Tackler 2 gate
 ...
 /// Maximum return touchdown - worst case for kicking team.
  ...
  #region Fumble Scenarios
      ...
      /// Injury checks included (tackle before fumble can cause injury).
 /// </summary>
   ...
   .KickoffOutOfBoundsCheck(0.5)
      .KickoffMuffCheck(0.99)
       .KickoffFairCatchCheck(0.9)
         .KickoffReturnYards(returnYardage)
           .BlockingPenaltyCheck(0.99)
       .FumbleCheck(0.01)       // Fumble triggers
     .TacklePenaltyCheck(0.99)
           // Injury checks
...
.TacklerInjuryGateCheck(0.9)
 ...
 /// </summary>
  ...
  .NextDouble(0.5)    // Kick distance
        .KickoffOutOfBoundsCheck(0.5)
           ...
           .KickoffFairCatchCheck(0.9)
        .KickoffReturnYards(returnYardage)
     .BlockingPenaltyCheck(0.01)        // Blocking penalty triggers
        .FumbleCheck(0.99)
       .TacklePenaltyCheck(0.99)
             // Injury checks
     .InjuryOccurredCheck(0.99)
 ...
 .KickoffFairCatchCheck(0.9)
  ...
  .BlockingPenaltyCheck(0.99)
     .FumbleCheck(0.99)
     .TacklePenaltyCheck(0.01)          // Tackle penalty triggers
           // Injury checks
        .InjuryOccurredCheck(0.99)
        .TacklerInjuryGateCheck(0.9)
           .TacklerInjuryGateCheck(0.9)
 ...
 bool isOnside = false,
          bool onsideRecovered = false)
        {
          var rng = new TestFluentSeedableRandom();
            ...
            if (isOnside)
       {
  // Onside kick path (no injuries on onside kicks - no returns/tackles)
            ...
            else
   {
     ...
     if (kickDistance >= 0.9)
     {
 // Touchback - short sequence (no injuries - no return)
       rng.KickoffOutOfBoundsCheck(0.5);
     }
       else
                {
       // Normal return path
  ...
  if (!muff)
   ...
   // Only add tackle penalty if not TD (simplified)
     ...
     // Injury checks (returner + tacklers)
