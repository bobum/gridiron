...
namespace UnitTestProject1
{
    ...
    public class FieldGoalPlayExecutionTests
    {
        ...
        [TestMethod]
        public void FieldGoal_Blocked_DefenseReturnsIntoOwnEndZone_Safety()
        {
            // Arrange
            var game = CreateGameWithFieldGoalPlay(isExtraPoint: false);
            game.FieldPosition = 85; // Far from own end zone, close to opponent's
            var play = (FieldGoalPlay)game.CurrentPlay;
            play.Possession = Possession.Home;
            game.HomeScore = 14;
            game.AwayScore = 7;
            ...
            var rng = new TestFluentSeedableRandom()
                ...
                .NextInt(1)        // BLOCKED!
                .NextDouble(0.3)   // Defense recovers
                .NextDouble(0.95)  // Ball bounces forward significantly (toward receiving end zone)
            ...
            if (play.IsSafety)
            {
                ...
                Assert.AreEqual(16, game.HomeScore, "Home (kicking team) should score safety (14 + 2)");
        ...
        [TestMethod]
        public void FieldGoal_Blocked_DefenseRecoversSafety_CorrectScoring()
        {
            // Arrange
            var game = CreateGameWithFieldGoalPlay(isExtraPoint: false);
            game.FieldPosition = 90; // Very far from own end zone
            var play = (FieldGoalPlay)game.CurrentPlay;
            ...
            var rng = new TestFluentSeedableRandom()
                ...
                .NextInt(1)        // BLOCKED!
                .NextDouble(0.3)   // Defense recovers
                .NextDouble(0.95)  // Ball bounces far forward
                .NextDouble(0.95)  // Random factor forward
                .NextDouble(0.5);  // Elapsed time
            ...
            // Assert - If safety occurred
            if (play.IsSafety)
            {
                Assert.IsTrue(play.Blocked, "Should be blocked");
                Assert.AreEqual(19, game.AwayScore, "Away (kicking team) should score safety (17 + 2)");
                Assert.AreEqual(21, game.HomeScore, "Home score should not change");
            }
        }
