...
namespace StateLibrary
{
    public class GameFlow
    {
        ...
        private void LogPlayStats()
        {
            var play = _game.CurrentPlay;
            if (play == null) return;

            if (play is PassPlay pp)
            {
                if (pp.PrimaryPasser != null)
                {
                    LogPlayerStats(pp.PrimaryPasser, "PASSING");
                }
                foreach (var segment in pp.PassSegments)
                {
                    if (segment.Receiver != null)
                    {
                        LogPlayerStats(segment.Receiver, "RECEIVING");
                    }
                }
            }
            else if (play is RunPlay rp)
            {
                foreach (var segment in rp.RunSegments)
                {
                    if (segment.BallCarrier != null)
                    {
                        LogPlayerStats(segment.BallCarrier, "RUSHING");
                    }
                }
            }
            else if (play is PuntPlay pup)
            {
                if (pup.Punter != null) LogPlayerStats(pup.Punter, "PUNTING");
                if (pup.InitialReturner != null) LogPlayerStats(pup.InitialReturner, "RETURNING");
            }
            else if (play is KickoffPlay kp)
            {
                if (kp.Kicker != null) LogPlayerStats(kp.Kicker, "KICKING");
                if (kp.InitialReturner != null) LogPlayerStats(kp.InitialReturner, "RETURNING");
            }
            else if (play is FieldGoalPlay fgp)
            {
                if (fgp.Kicker != null) LogPlayerStats(fgp.Kicker, "KICKING");
            }
        }

        private void LogPlayerStats(Player p, string type)
        {
            ...
            if (type == "PASSING")
            {
                ...
                var yds = p.Stats.ContainsKey(StatTypes.PlayerStatType.PassingYards) ? p.Stats[StatTypes.PlayerStatType.PassingYards] : 0;
                ...
                statsLine = $"{comp}/{att}, {yds} yds, {tds} TD, {ints} INT";
            }
            else if (type == "RUSHING")
            {
                ...
                var yds = p.Stats.ContainsKey(StatTypes.PlayerStatType.RushingYards) ? p.Stats[StatTypes.PlayerStatType.RushingYards] : 0;
                ...
                statsLine = $"{att} att, {yds} yds, {tds} TD";
            }
            else if (type == "RECEIVING")
            {
                var rec = p.Stats.ContainsKey(StatTypes.PlayerStatType.Receptions) ? p.Stats[StatTypes.PlayerStatType.Receptions] : 0;
                var yds = p.Stats.ContainsKey(StatTypes.PlayerStatType.ReceivingYards) ? p.Stats[StatTypes.PlayerStatType.ReceivingYards] : 0;
                ...
                statsLine = $"{rec} rec, {yds} yds, {tds} TD";
            }
             else if (type == "PUNTING")
            {
                var punts = p.Stats.ContainsKey(StatTypes.PlayerStatType.Punts) ? p.Stats[StatTypes.PlayerStatType.Punts] : 0;
                var yds = p.Stats.ContainsKey(StatTypes.PlayerStatType.PuntYards) ? p.Stats[StatTypes.PlayerStatType.PuntYards] : 0;
                var inside20 = p.Stats.ContainsKey(StatTypes.PlayerStatType.PuntsInside20) ? p.Stats[StatTypes.PlayerStatType.PuntsInside20] : 0;
                var avg = punts > 0 ? (double)yds / punts : 0;
                statsLine = $"{punts} punts, {yds} yds ({avg:F1} avg), {inside20} in 20";
            }
            else if (type == "KICKING")
            {
                // Field Goals
                if (p.Stats.ContainsKey(StatTypes.PlayerStatType.FieldGoalsAttempted))
                {
                    var made = p.Stats.ContainsKey(StatTypes.PlayerStatType.FieldGoalsMade) ? p.Stats[StatTypes.PlayerStatType.FieldGoalsMade] : 0;
                    var att = p.Stats[StatTypes.PlayerStatType.FieldGoalsAttempted];
                    statsLine = $"FG: {made}/{att}";
                }
            }

            if (!string.IsNullOrEmpty(statsLine))
            {
                _logger.LogInformation($"   > {p.LastName} ({type}): {statsLine}");
            }
        }
